...
function parseMarkdownToHtml(markdown: string) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('```')) {
      ...
      htmlLines.push(inCodeBlock ? '<pre class="bg-[#0E332C] text-[#2563EB] p-3 rounded-xl border border-[#23574B] font-mono text-[11px] overflow-x-auto my-2">' : '</pre>');
      continue;
    }
    ...
    if (trimmed.startsWith('|') && i + 1 < lines.length && isTableSeparatorRow(lines[i + 1].trim())) {
      ...
      while (i < lines.length && lines[i].trim().startsWith('|')) {
        ...
        bodyRowsHtml.push(
          `<tr class="border-b border-[#23574B]">${cells.map(c => `<td class="px-2.5 py-1.5 align-top">${parseInlineMarkdown(c)}</td>`).join('')}</tr>`
        );
        i++;
      }
      i--; // step back one since the outer for-loop will increment

      const theadHtml = `<thead><tr class="bg-[#0E332C] border-b-2 border-[#23574B]">${headerCells.map(c => `<th class="px-2.5 py-1.5 text-left font-display font-semibold text-[#2563EB]">${parseInlineMarkdown(c)}</th>`).join('')}</tr></thead>`;
      htmlLines.push(
        `<div class="overflow-x-auto my-3 rounded-lg border border-[#23574B]"><table class="w-full text-[11px] border-collapse">${theadHtml}<tbody>${bodyRowsHtml.join('')}</tbody></table></div>`
      );
      continue;
    }
    ...
    if (trimmed.startsWith('## ')) { htmlLines.push(`<h3 class="font-display font-bold text-[#F1F5F9] text-base mt-6 mb-3 border-b border-[#23574B] pb-1">${trimmed.replace('## ', '')}</h3>`); continue; }
...
export default function ChatAssistant({ startupProfile, onSelectGrantFromChat, currentLanguage = 'english' }: ChatAssistantProps) {
  ...
  return (
    <div className="bg-[#0E332C] border border-[#23574B] rounded-2xl shadow-sm flex flex-col h-[780px] relative overflow-hidden">

      {/* Header */}
      <div className="bg-[#0E332C] border-b border-[#23574B] px-5 py-4 flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-lg bg-[#0E332C] border border-[#23574B] flex items-center justify-center">
        ...
        <div className="flex items-center gap-1.5 bg-[#0E332C] px-2.5 py-1 rounded-full text-[10px] font-mono text-[#2563EB] border border-[#23574B]">
      ...
      {/* Messages */}
      <div ref={messagesContainerRef} className="flex-1 overflow-y-auto px-5 py-6 space-y-4 bg-[#0E332C]/30">
        {messages.map((msg) => (
          <div key={msg.id} className={`flex items-start gap-3 max-w-[85%] ${msg.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
            <div className={`w-8 h-8 rounded-lg shrink-0 flex items-center justify-center border text-xs font-mono font-bold ${msg.role === 'user' ? 'bg-[#3B82F6] border-transparent text-white' : 'bg-[#0E332C] border-[#23574B] text-[#3B82F6]'}`}>
              {msg.role === 'user' ? <User className="w-4 h-4" /> : <Bot className="w-4 h-4" />}
            </div>
            <div className="space-y-1">
              <div className={`p-3.5 rounded-2xl text-xs leading-relaxed ${msg.role === 'user' ? 'bg-[#3B82F6] text-white rounded-tr-none border border-[#2563EB]/15 shadow-sm' : 'bg-[#0E332C] text-slate-100 rounded-tl-none border border-[#23574B] shadow-sm'}`}>
                {msg.role === 'user' ? (
              ...
              <div className="flex items-center justify-between gap-4 mt-1.5 min-w-[120px]">
                {msg.role === 'assistant' ? (
                  <button
                    ...
                    className="flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] text-[#3B82F6] hover:text-[#2563EB] hover:bg-[#0E332C]/50 font-mono font-bold transition cursor-pointer"
                  >
        ...
        {loading && (
          <div className="flex items-start gap-3 max-w-[85%]">
            <div className="w-8 h-8 rounded-lg bg-[#0E332C] border border-[#23574B] flex items-center justify-center shrink-0">
              <Bot className="w-4 h-4 text-[#3B82F6] animate-bounce" />
            </div>
            <div className="space-y-2 bg-[#0E332C] border border-[#23574B] p-4 rounded-2xl rounded-tl-none w-full max-w-md shadow-sm">
              <div className="flex items-center gap-2 text-[10px] font-mono text-[#3B82F6] border-b border-[#23574B] pb-2 mb-2">
      ...
      {messages.length <= 2 && (
        <div className="px-5 py-3 border-t border-[#23574B] bg-[#0E332C]">
          <p className="text-[10px] font-mono font-bold uppercase tracking-wider text-[#94A3B8] mb-2">
            {t.suggestedQueries}
          </p>
          <div className="flex flex-wrap gap-1.5">
            {quickPrompts.map((qp, idx) => (
              <button key={idx} onClick={() => handleSend(qp.text)} className="bg-[#0E332C] border border-[#23574B] text-slate-200 hover:text-[#3B82F6] hover:border-[#3B82F6] text-[11px] px-2.5 py-1.5 rounded-xl transition flex items-center gap-1.5 cursor-pointer shadow-sm">
                <span>{qp.icon}</span>
                <span>{qp.text}</span>
              </button>
            ))}
      ...
      {/* Input */}
      <div className="p-4 bg-[#0E332C] border-t border-[#23574B] flex items-center gap-2">
        <button type="button" onClick={toggleListening} disabled={loading} className={`shrink-0 w-10 h-10 rounded-xl flex items-center justify-center border transition cursor-pointer ${isListening ? 'bg-rose-500 border-rose-600 text-white animate-pulse' : 'bg-[#0E332C] border-[#23574B] text-[#3B82F6] hover:bg-[#0E332C]'}`}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleSend(input); }} className="flex-1 flex gap-2 relative">
          <input
            type="text"
            className="flex-1 bg-[#0E332C] text-[#F1F5F9] text-xs px-4 py-3 rounded-xl border border-[#23574B] focus:outline-none focus:border-[#3B82F6] transition pr-16"
