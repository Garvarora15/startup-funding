...
export default function CollapsibleFAQ({ currentLanguage }: CollapsibleFAQProps) {
  ...
  return (
    <div id="faq-section" className="bg-[#0E332C] border border-[#23574B] rounded-2xl p-6 shadow-sm mt-8">
      ...
      <div className="space-y-2.5">
        {faqs.map((faq, idx) => {
          ...
          return (
            <div 
              key={idx} 
              className="border border-[#23574B] rounded-xl overflow-hidden transition-colors duration-200"
            >
              <button
                type="button"
                onClick={() => setOpenIdx(isOpen ? null : idx)}
                className="w-full flex items-center justify-between p-4 bg-[#0E332C] hover:bg-[#0E332C] text-left transition cursor-pointer"
              >
                <span className="font-sans font-semibold text-[#2563EB] text-xs md:text-sm">
                  {faq.q}
                </span>
                {isOpen ? (
                  <ChevronUp className="w-4 h-4 text-[#3B82F6] shrink-0 ml-2" />
                ) : (
                  <ChevronDown className="w-4 h-4 text-[#3B82F6] shrink-0 ml-2" />
                )}
              </button>
              {isOpen && (
                <div className="p-4 bg-[#0E332C] border-t border-[#23574B] font-sans text-xs text-slate-300 leading-relaxed">
                  {faq.a}
                </div>
              )}
            </div>
